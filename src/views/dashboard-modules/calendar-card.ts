...
function renderMonth(body: HTMLElement, context: DashboardModuleRenderContext): void {
	body.empty();
	...
	body.createDiv({
		cls: 'op-module-calendar-caption',
		text: `${model.year} 年 ${model.month + 1} 月`,
	});
...
function renderCalendar(context: DashboardModuleRenderContext): void {
	...
	const rerender = () => renderMonth(body, context);
	...
	renderMonth(body, context);
	const today = body.querySelector<HTMLElement>('.is-today');
	if (today) setIcon(today.createSpan({ cls: 'op-module-calendar-today-dot', attr: { 'aria-hidden': 'true' } }), 'dot');
}
