...
- 可拖拽、缩放和独立配置的个人工作台，支持任务、天气、日期、日历、待办、笔记统计、最近文件、资讯、目录、Markdown 文本、图表、倒计日、文件变更热力图和网页 iframe；每张卡片可设置背景颜色。
- **设置 → 个人仪表盘** 可控制允许新增的卡片类型，并统一配置和风天气与 OpenWeatherMap 凭据。
- 个人卡片支持右键复制；日历支持农历、天干地支、传统节日、24 节气、今日及周末标记。
- 待办可在卡片中直接完成；百分比支持手工数据与进度条；笔记统计支持后缀和元数据筛选；文件卡片支持最近创建、最近编辑和常用文件。
- 普通关联、项目内父子任务和可编辑备注。
- 结构化子任务保存在父任务 Markdown 文件中，支持独立新增/编辑弹窗、优先级、经办人、开始/计划日期、标签及完成状态。
...
向 GitHub 推送与版本完全一致的标签后，Release 工作流会自动运行测试、lint、类型检查和生产构建，生成构建来源证明，并公开发布 `main.js`、`manifest.json`、`styles.css`。例如：
...
- 网页卡片仅接受 HTTP/HTTPS 地址，并在带 `sandbox`、`no-referrer` 和延迟加载限制的 iframe 中打开。加载网页会让目标站点收到常规浏览器网络请求并执行其自身脚本；插件不会主动向目标站点发送 Vault、笔记或项目内容。只应嵌入可信站点，目标站点也可能通过 CSP 或 `X-Frame-Options` 拒绝显示。
- 插件只通过 Obsidian API 读写当前 Vault。
- 资讯与自定义天气主机仅允许公共 HTTPS 地址，并统一应用超时和并发请求合并；本机、回环、私网及链路本地字面地址会被拒绝。
- Markdown 是业务主数据；`data.json` 只保存启动路径和设备 UI 状态。
- **设置 → 配置数据** 支持导入/导出项目配置；导出默认排除天气 API 密钥，导入会先验证和预览，写入失败自动恢复旧配置。
